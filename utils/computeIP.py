...
class straight_line(object):
    ...
    Straight line is described by an origin and a direction: point = origin + direction * par, where par is a number that parametrizes the line.

    It includes methods to get the point given a certain parameter, and to find the minimum approach between the line and an external point.
    """

    def __init__(self, origin, direction):
        """Simple constructor with 2 TVector3 objects, origin and direction (which is then normalized)."""
    ...
    def distance_from_point(self, point):
        """Given an external point, returns the TVector3 starting from this point and going to the line point of closest approach to the external one.
...
def compute_IP_wrt_direction(helix, primary_vertex, jet_direction, debug = False):
    """Given a helix object, compute the impact parameter with respect to a given direction, as illustrated in the following note: D. Brown, M. Frank, Tagging b hadrons using impact parameters, ALEPH note 92-135.
    ...
    This version of the impact parameter is more complicated than the common one, because it computes firstly the minimum approach between the helix and the jet direction, then the track is linearized at this point, and the impact parameter is computed as the closest approach between the linearized track and the primary vertex.

    In the code you can find a few comments with the names of the variables used in the ALEPH note.
    """
    ...
    helix.min_approach = minimize_scalar(jet_track_distance, bracket = None, bounds = [-1e-11, 1e-11], args=(), method='bounded', tol=None, options={'disp': 0, 'maxiter': 1e5, 'xatol': 1e-20} )
...
def compute_IP(helix, primary_vertex, jet_direction):
    """ Given a helix object and a primary vertex, compute the impact parameter with respect to the primary vertex.

    The impact parameter is the vector of closest approach between the helix and the primary vertex. The jet direction is used only to give a sign to the IP.
    Primary vertex and jet direction must be TVector3 objects.

    The function returns the vector IP (pointing from the primary vertex to the helix point of closest approach), the sign and the IP (that is the magnitude of the vector IP with the proper sign) as attributes of the helix object.
    """
    ...
    helix.min_approach = minimize_scalar(pr_vertex_track_distance, bracket = None, bounds = [-1e-11, 1e-11], args=(), method='bounded', tol=None, options={'disp': 0, 'maxiter': 1e5, 'xatol': 1e-20} )
    helix.min_approach_time = helix.min_approach.x
