...
class LeptonSmearer(Analyzer):
    ...
    * output: output particle collection.
    ...
    def smear_electron(self, obj):
        ...
        Gaussian smearing of the p4 by 3%.
        Electron accepted if smeared energy> 0.5 and |eta|<2.5
        '''
        smeared = self.smear(obj, 1, 0.03)
        if abs(smeared.eta())<2.5 and smeared.e()> 0.5:
            return smeared
        else:
            return None
    ...
    def smear_muon(self, obj):
        ...
        Electron accepted if smeared energy> 0.2. and |eta|<2
        '''
        smeared = self.smear(obj, 1, 0.004)
        if abs(smeared.eta())<2. and smeared.e()> 0.2:
            return smeared
        else:
            return None
