...
cebefo_style()
save = True
dir_fig = "fig/tracks/with_no_cut/sp_IP/"
...
my_file = TFile("ntuple/outdir_sp/qq/analyzers.ZqqIPTrackTreeProducer.ZqqIPTrackTreeProducer_2/tree.root","OPEN")
...
# ip smeared
impact_parameter_smeared = QuarksDistroPlotter(my_tree, "ip_smeared", "", "ip_smeared_{}".format(detector)+mode_save, 200, -2e-3, 5e-3, "IP_{} @{}".format("{smeared}", detector) + mode, "IP_{smeared} [m]", "m", norm = False, logy = True, set_title = True)
...
# track Significance
track_significance_logy = QuarksDistroPlotter(my_tree, "signif", "", "track_significance_logy_{}".format(detector)+mode_save, 200, -10, 50, "Track significance @{}".format(detector) + mode, "Track significance", "", norm = False, logy = True, set_title = True)

# track probability
track_probability = QuarksDistroPlotter(my_tree, "track_probability", "", "track_probability_{}".format(detector)+mode_save, 201, -1, 1, "Track probability @{}".format(detector) + mode, "Track probability", "", norm = False, logy = True, set_title = True)
