...
variable = "(mc_quark_nearest_rec_jet_e - mc_quark_nearest_gen_jet_e)/mc_quark_nearest_gen_jet_e"
title = "Zed jets energy resolution"
xtitle = "(E_{rec} - E_{gen})/E_{gen}"
...
for tree, form, legend_name in zip(trees, formats, legend_names):
    ...
    histogram.GetYaxis().SetTitle("Entries/ {:3.2f}".format(histogram.GetBinWidth(1)))
    histogram.GetXaxis().SetTitle(xtitle)
    tree.Project(histogram.GetName(), variable)
    histogram.SetLineColor(len(histogram_list) + 2)
    histogram.Fit("gaus", "0", "", -0.04, 0.1)
    histogram.Fit("gaus", "0", "", -histogram.GetFunction("gaus").GetParameter(2)*2, 0.1)
    histogram.Fit("gaus", "0", "", -histogram.GetFunction("gaus").GetParameter(2)*2, 0.1)
